...
function ActiveWorkoutScreen({ workout, setWorkout, exercises, unit, updateExercise, createExercise, onFinish, onCancel, floatingTimer, setFloatingTimer, playAlarm, unlockAudio }) {
  ...
  const addExerciseToWorkout = (exerciseId) => {
    ...
    const initialSets = lastSets.length > 0
      ? lastSets.map((s) => ({ weight: String(s.weight), reps: String(s.reps), completed: false }))
...
// ---------- List Mode ----------
// FIX 3: inline edit panel for rep range and rest time, opened by tapping the
// exercise name / sub-line inside a workout, without navigating away.

function ListModeView({ workout, exercises, unit, updateSet, completeSet, addSet, removeSet, removeExerciseFromWorkout, startTimer, updateExercise }) {
  const [editingEntry, setEditingEntry] = useState(null); // entryIndex being edited
  ...
  return (
    <div className="list-mode">
      {workout.entries.map((entry, entryIndex) => {
        ...
        if (!ex) return null;
        const lastSession = ex.history[ex.history.length - 1];
        const isEditing = editingEntry === entryIndex;
            ...
            <div className="exercise-block-header">
              <div className="exercise-block-title-group" style={{ flex: 1 }}>
                ...
                <div className="exercise-block-sub">
                  {ex.repRangeLow}–{ex.repRangeHigh} reps · {ex.restSeconds}s rest
                </div>
              </div>
              <button
                className="icon-btn-ghost"
                onClick={() => setEditingEntry(isEditing ? null : entryIndex)}
                title="Edit rep range / rest"
              >
                <Edit2 size={15} />
              </button>
                ...
                <div className="set-row-wrap" key={setIndex}>
                  <div className={`set-row ${set.completed ? "set-row-done" : ""}`}>
                    ...
                    <input
                      ...
                      type="number"
                      inputMode="decimal"
                      placeholder={lastSet ? fmtNum(lastSet.weight) : "0"}
                      value={set.weight}
                  ...
                  {lastSet && (
                    <div className="set-last-hint">
                      Last time: {fmtNum(lastSet.weight)} × {lastSet.reps}
                    </div>
                  )}
                </div>
                );
...
// FIX 3: Inline editor component
function InlineExerciseEditor({ exercise, onUpdate, onClose }) {
  ...
  const commit = () => {
    ...
    if (!isNaN(parsedRest) && parsedRest > 0) patch.restSeconds = parsedRest;
  ...
  return (
    <div className="inline-edit-panel">
      <div className="inline-edit-row">
        ...
        <div className="inline-edit-field">
          <label className="field-label">Rest (s)</label>
          <input
            className="text-input inline-edit-input"
            type="number"
            inputMode="numeric"
