...
export default function App() {
  ...
  // FIX 1: unlockAudio — must be called synchronously from a tap handler
  // so iOS Safari allows the AudioContext to be created/resumed.
  ...
  const playAlarm = useCallback(() => {
    ...
    // navigator.vibrate not supported on iOS — skip silently
  ...
  // CHANGE 1 + 2: Build each exercise's starting sets from its history.
  // Same SET COUNT as last session, and each set is prefilled with that
  // set's last-time weight/reps as live (editable) values, plus we tag
  // it "pristine" so completeSet knows it can be logged untouched.
  const buildSetsFromHistory = (exercise) => {
  ...
  const finishWorkout = (workout) => {
    ...
    // Detect if the workout differs from its source template (exercises or set counts changed)
    ...
    if (workout.fromTemplateId && data.templates[workout.fromTemplateId]) {
      ...
      if (tmplIds.join(",") !== workoutIds.join(",")) {
        ...
        // check if set counts differ from the last session baseline
        workout.entries.forEach((entry, i) => {
          const ex = data.exercises[entry.exerciseId];
          const lastSession = ex?.history?.[ex.history.length - 1];
          const baseCount = lastSession ? lastSession.sets.length : 1;
...
function ActiveWorkoutScreen({ workout, setWorkout, exercises, unit, updateExercise, createExercise, buildSetsFromHistory, onFinish, onCancel, floatingTimer, setFloatingTimer, playAlarm, unlockAudio }) {
  ...
  // FIX 1: unlockAudio is called synchronously from startTimer (which is always
  // triggered by a user tap), priming the AudioContext for the later alarm.
  const startTimer = (exerciseId, seconds) => {
    unlockAudio(); // must be in the tap handler call stack
    setFloatingTimer({
  ...
  const addSet = (entryIndex) => {
    setWorkout((w) => {
      ...
      entries[entryIndex] = {
        ...entries[entryIndex],
        // CHANGE 1: extra sets beyond history start empty, not prefilled.
  ...
  // CHANGE 2: editing a box clears "pristine" so we know the last-time
  // value has been overridden by the person.
  ...
  const addExerciseToWorkout = (exerciseId) => {
    ...
    // CHANGE 1: use the same history-based set builder when adding an
    // exercise mid-workout too, so behavior is consistent everywhere.
    const initialSets = buildSetsFromHistory(ex);
    setWorkout((w) => ({
      ...w,
...
// ---------- List Mode ----------
// CHANGE 3: the pencil icon now opens ONLY a rep-range editor.
// Rest time editing moved to tapping the orange "Rest Xs" pill at the
// bottom-right of each exercise card. The "· Xs rest" text next to the
// rep range up top is removed. Manual rest-timer-start is removed —
// rest only ever starts automatically when a set is completed.

function ListModeView({ workout, exercises, unit, updateSet, completeSet, addSet, removeSet, removeExerciseFromWorkout, updateExercise }) {
  const [editingRepRange, setEditingRepRange] = useState(null); // entryIndex
  const [editingRest, setEditingRest] = useState(null); // entryIndex
        ...
        return (
          <div className="exercise-block" key={entryIndex}>
              ...
              {entry.sets.map((set, setIndex) => (
                <div className="set-row-wrap" key={setIndex}>
                  <div className={`set-row ${set.completed ? "set-row-done" : ""}`}>
                    ...
                    <input
                      className={`set-input ${set.pristine ? "set-input-prefilled" : ""}`}
                      type="number"
                      inputMode="decimal"
                      placeholder="0"
                      value={set.weight}
            ...
            <div className="exercise-block-actions">
              ...
              >
                Rest {ex.restSeconds < 60 ? `${ex.restSeconds}s` : ex.restSeconds % 60 === 0 ? `${ex.restSeconds / 60}m` : `${(ex.restSeconds / 60).toFixed(1)}m`}
              </button>
            </div>
...
// CHANGE 3: pencil-triggered editor — rep range only.
function InlineRepRangeEditor({ exercise, onUpdate, onClose }) {
...
// Rest time editor — simple minutes input, no presets.
...
function FocusModeView({ workout, exercises, completeSet, updateSet, addSet, removeSet, updateExercise }) {
...
function FinishSummaryModal({ summary, exercises, unit, onClose, onSaveAsTemplate, onUpdateTemplate }) {
  ...
  const [step, setStep] = useState("main"); // "main" | "save-template" | "name-workout"

  const totalSets = summary.workout.entries.reduce((sum, e) => sum + e.sets.length, 0);

  // After viewing summary: if new workout → ask to name it and save as template
  // If template was modified → ask to update template
  const handleDone = () => {
...
// ---------- Modal wrapper — tall pull-up sheet, pull down to dismiss ----------
...
/* Inline edit panels (rep range via pencil, rest via orange pill) */
.inline-edit-panel {
  background: var(--bg-elevated);
...
/* CHANGE 2: visually distinguish a box still showing last time's
   untouched value from one the person has actually typed into. */
.set-input-prefilled {
  color: var(--text-dim);
...
/* CHANGE 3: orange rest pill, bottom-right of each exercise card.
   Tapping opens the rest-time editor; it no longer starts a timer. */
.rest-pill-btn {
  background: var(--accent-dim);
  border: 1px solid var(--accent-dim);
  color: var(--accent);
...
/* FIX 2: modal scrolling — -webkit-overflow-scrolling for iOS momentum scroll */
.modal-backdrop {
  position: fixed;
  inset: 0;
...
.focus-set-controls { display: flex; gap: 20px; align-items: center; margin-bottom: 10px; }
