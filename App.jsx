...
// CHANGE 3: orange-pill-triggered editor — rest time only.
function InlineRestEditor({ exercise, onUpdate, onClose }) {
  ...
  const commit = () => {
    ...
    if (!isNaN(parsedRest) && parsedRest > 0) onUpdate({ restSeconds: parsedRest });
    onClose();
  };
  ...
  return (
    <div className="inline-edit-panel inline-edit-panel-rest">
      <div className="inline-edit-row">
        <div className="inline-edit-field">
          <label className="field-label">Rest time (seconds)</label>
          <input
            className="text-input inline-edit-input"
            type="number"
            inputMode="numeric"
      ...
      <div className="rest-preset-row">
        {presets.map((p) => (
          <button
            key={p}
            type="button"
            className={`rest-preset-chip ${parseInt(rest) === p ? "rest-preset-chip-active" : ""}`}
            onClick={() => setRest(String(p))}
          >
            {p}s
          </button>
        ))}
...
// ---------- Modal wrapper (drag up to expand, drag down to dismiss) ----------
...
function Modal({ children, onClose, title }) {
  ...
  const onTouchStart = (e) => {
    ...
    if (touch.clientY - sheetTop > 44) return;
  ...
  const onTouchMove = (e) => {
    ...
    const maxUp = -(window.innerHeight * 0.45);
    const clamped = Math.max(next, maxUp);
