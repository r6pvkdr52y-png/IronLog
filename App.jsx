  ...
  return (
    <div className="app-shell" style={{
      ...
      <div className="app-content">
        ...
        {tab === "graphs" && (
          <GraphsScreen exercises={data.exercises} workouts={data.workouts} unit={data.settings.unit} onSelect={(id) => setViewingExercise(id)} />
        )}
        {tab === "records" && (
...
function GraphsScreen({ exercises, workouts, unit, onSelect }) {
  const [view, setView] = useState("progress");

  const list = Object.values(exercises)
    .filter((e) => e.history.filter(h => !h.isDeload).length > 0)
...
function ActiveWorkoutScreen({ workout, setWorkout, exercises, unit, theme, updateExercise, createExercise, buildSetsFromHistory, onFinish, onCancel, floatingTimer, setFloatingTimer, playAlarm, unlockAudio }) {
  ...
  const completeSet = (entryIndex, setIndex) => {
    ...
    if (ex) startTimer(entry.exerciseId, restSeconds);
  };
...
function ListModeView({ workout, exercises, unit, updateSet, completeSet, addSet, removeSet, addDropSet, removeDropSet, removeExerciseFromWorkout, moveExercise, updateExercise }) {
